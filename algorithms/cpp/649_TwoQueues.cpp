...
public:
    string predictPartyVictory(string senate) {
        ...
        // Push the surviving senators at the end of the queue
        // Pop both fronts of the queue
        while(!radQueue.empty() && !dirQueue.empty()) {
            if(radQueue.front() < dirQueue.front()) {
                radQueue.push(radQueue.front() + n);
            }
            else {
                dirQueue.push(dirQueue.front() + n);
            }
            radQueue.pop();
            dirQueue.pop();
        } 
        // If the queue is empty -> that faction lost
        if(radQueue.empty()) {
            return "Dire";
        }
        return "Radiant";
